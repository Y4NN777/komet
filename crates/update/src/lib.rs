//! komet-update — release checking and self-update, shared by the engine (the
...
/// `komet-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("komet-{version}-{os}-{arch}.tar.gz")
}

/// `komet-<ver>-macos-<arch>-app.tar.gz` — the macOS app update payload.
...
fn http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent(concat!("komet/", env!("CARGO_PKG_VERSION")))
        .build()
...
pub enum InstallKind {
    /// `~/.komet/app/<ver>/komet` behind the `current` symlink
    /// (curl|sh installer / a previous `komet update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".komet").join("app");
...
pub async fn stage_headless(
    ...
    let result = async {
        ...
        if !unpacked.join("komet").is_file() {
            bail!("tarball {file} did not contain a komet binary");
        }
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("komet").exists() => {}
            Err(err) => {
...
/// Restart the installed engine service (the same units `komet daemon` and the
...
pub fn restart_service() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/sh.komet.app")],
        )
    } else {
        run("systemctl", &["--user", "restart", "komet.service"])
    }
}
...
pub async fn stage_mac_app(
    ...
    if staged.join("Contents/MacOS/komet").exists() {
        return Ok(staged);
    }
    ...
    if !staged.join("Contents/MacOS/komet").exists() {
        bail!("app tarball {file} did not contain Komet.app");
    }
    Ok(staged)
}
...
/// `KOMET_AUTO_UPDATE=1|true|yes` — headless daemons apply updates themselves.
fn auto_update_enabled() -> bool {
    std::env::var("KOMET_AUTO_UPDATE")
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.komet/app/0.1.1/komet"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.komet/app")
            }
        );
        assert_eq!(
            detect_install_from(
                Path::new("/Applications/Komet.app/Contents/MacOS/komet"),
                Some(Path::new("/Users/u")),
            ),
            InstallKind::MacApp {
                bundle: PathBuf::from("/Applications/Komet.app")
            }
        );
        // A path merely containing `.app` without the bundle layout is not a bundle.
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/komet"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/komet"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("komet-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("komet-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"komet-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["komet-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("komet"), ver).unwrap();
        }
