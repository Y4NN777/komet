...
fn cline_fixture_path() -> PathBuf {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures")
        .join("fake-cline-acp.sh");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let _ = std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o755));
...
/// Cline harness pointed at the fake Cline ACP server. The fixture reads the
/// scenario from its own first positional arg (`--acp` is what the spec
/// passes, like every ACP launch), so each scenario gets a tiny generated
/// shim — no shared process-env races (Rust test threads share one env, and
/// a global `SCENARIO` would leak across parallel tests).
fn cline_harness(scenario: &str) -> AcpHarness {
    let fixture = cline_fixture_path();
    let path = fixture
        .parent()
        .expect("fixture dir")
        .join(format!("fake-cline-acp-{scenario}.sh"));
    let body = format!(
        "#!/bin/sh\n# Generated by acp.rs: pins SCENARIO for the Cline fixture.\n# Forwards argv through (the spec passes --acp, like every ACP launch).\nSCENARIO={scenario} exec \"{fixture}\" \"$@\"\n",
        scenario = scenario,
        fixture = fixture.display(),
    );
    std::fs::write(&path, body).expect("write cline scenario shim");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o755))
            .expect("chmod cline scenario shim");
    }
    AcpHarness::cline().with_executable(path)
}
...
/// Cline's ACP surface: native `cline --acp`, turn-boundary steering, no
/// advertised `thought_level` (its `--thinking` CLI tiers have no ACP
/// equivalent — verified against `session/new` on cline 3.0.62).
#[test]
fn cline_descriptor_surface_matches_registry_expectations() {
    let harness = AcpHarness::cline();
    assert_eq!(harness.id(), HarnessId::Cline);
    assert_eq!(harness.display_name(), "Cline");
    assert!(harness.supports_steering());
    assert_eq!(harness.steering_mode(), SteeringMode::TurnBoundary);
...
/// Cline's two-tier model-select contract: when Cline advertises the `provider`
/// select (cline / cline-pass / openai-codex) ahead of the real `model` select
/// — both with the `model` category — discovery must pick the option actually
/// named `model`, not the first category match (verified live against
/// `cline --acp` 3.0.62). A scripted process stands in for the CLI.
#[tokio::test]
async fn cline_model_select_wins_over_the_provider_select() {
    let harness = cline_harness("two-tier");
    let models = harness.models().await.expect("discovery");
    let ids: Vec<&str> = models.iter().map(|m| m.id.as_str()).collect();
    assert_eq!(ids.len(), 301, "{ids:?}");
    assert!(ids.contains(&"some/model-001"));
    assert!(
        !ids.iter()
    ...
    let provider = models[0]
        ...
        .expect("provider must surface as a trait");
    assert_eq!(
        provider
            .choices
            .iter()
...
/// The single-select variant (no `provider`) keeps the first-match
/// behaviour the existing agents rely on.
#[tokio::test]
async fn cline_single_model_select_still_discovered() {
    let harness = cline_harness("model-only");
    let models = harness.models().await.expect("discovery");
    let ids: Vec<&str> = models.iter().map(|m| m.id.as_str()).collect();
    assert_eq!(ids, vec!["a/b"]);
}

/// An unsigned-in Cline rejects `session/new` with -32000. Discovery must
/// surface an error carrying the agent's own wording (the static catalog is
/// NOT silently substituted) so the settings page can tell the user to sign
/// in with `cline` first.
#[tokio::test]
async fn cline_unauthenticated_discovery_fails_loudly() {
    let harness = cline_harness("unauthed");
    let err = harness
        .models()
        .await
        .expect_err("auth must fail discovery");
